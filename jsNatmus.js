...
function Natmus() {
    this.CIP_BASE = "http://samlinger.natmus.dk/CIP/";
    ...
    this.ciprequest = function(name, options, success, error) {
        ...
        return qwest.post(this.CIP_BASE + name + ";jsessionid=" + this.jsessionid, queryStringObject, {async: false})
    ...
    this.session_open = function(username, password, successCallback) {
        var self = this; // TODO: fix this hack

        qwest.post(this.CIP_BASE + "session/open", {user: username, password: password})
    ...
    this.get_layout = function(collection, success) {
        var retval = this.ciprequest("metadata/getlayout/"+collection, {}, function(response) {
            success(response);
        });
